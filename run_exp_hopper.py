...
from baseline_scripts.DDPG import Actor, Critic
from baseline_scripts.utils import ReplayBuffer
import torch
...
def get_models(files, n=10, top_q_count=2):
    ...
    for i, v in enumerate(model_values):
        if v >= TOP_Q_FLOOR:
            ...
            actor.load_state_dict(torch.load(model_path + files[i]))
            critic = Critic(state_dim, action_dim).to(device)
            critic.load_state_dict(torch.load(model_path + files[i].replace('actor', 'critic')))
            selected_model_functions.append((actor, critic))
...
def experiment2(num_model, data_size, num_runs, data_explore_rate, resolutions):
    ...
    for run in range(num_runs):
        ...
        data_names = get_file_names([ENV_NAME, 'action', str(data_explore_rate)], data_path)
        ...
        bvft = BVFT(q_functions, dataset, GAMMA, RMAX, RMIN, record, bins=bins, q_type=q_type, data_size=data_size)
...
def run_experiment_2(num_runs):
    ...
    resolutions = {5000: [0.01, 0.1, 0.2, 0.5, 0.7, 1.0],
                   500: [0.01, 0.1, 0.2, 0.5, 0.7, 1.0],
                   50000: [0.01, 0.1, 0.2, 0.5, 0.7, 1.0]}
    for k in resolutions:
        resolutions[k] = np.array(resolutions[k]) * 50
...
def experiment4(num_model):
    ...
    for j, data_size in enumerate(data_sizes):
        ...
        print(q_names)
        record.q_names = q_names
        bvft = BVFT(q_functions, dataset, GAMMA, RMAX, RMIN, record, bins=bins, q_type= q_type, verbose=True, data_size=data_size)
...
if __name__ == '__main__':
    ...
    q_type = 'torch_actor_critic_cont'
    model_path = "./models/"
    data_path = "./buffers/"
    GAMMA = 0.99
    RMAX, RMIN = 10.0, -10.0
    ...
    explore_rate = 0.3
    model_keywords = ["DDPG", ENV_NAME, "actor"]
    data_keywords = [ENV_NAME, "action", str(explore_rate)]
    data_sizes = [1000, 10000]
    # data_sizes = [5]

    resolutions = np.array([0.1, 0.2, 0.5, 0.7, 1.0]) * 100

    model_counts = [10]*2
    TOP_Q_FLOOR = 2500
    NORMAL_Q_CEILING = 2200
    NORMAL_Q_FLOOR = 800
    model_gap = 50.0
