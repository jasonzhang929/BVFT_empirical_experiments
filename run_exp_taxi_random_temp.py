    ...
    for data_explore_rate in data_explore_rates:
        ...
        for i, data_size in enumerate(data_sizes):
            ...
            plot_top_k_metrics(axs[i][:2], matched_records, exclude_q_star=not include_q_star, plot_loc=plot_loc,
                               auto_res=False, include_avgqsa=False)
            if auto_res:
                plot_top_k_metrics(axs[i][2:], matched_records, exclude_q_star=not include_q_star, plot_loc=plot_loc,
                                   auto_res=auto_res, include_avgqsa=False)
...
def fill_bellman_error():
    ...
    for file in record_files:
        ...
        for i, record in enumerate(records):
            ...
            # m = np.load(dir_path + F"/data/{ENV_NAME}/" + record.q_names[0])
            # print(np.max(m), np.min(m), np.max(m) - np.min(m))
...
if __name__ == '__main__':
    ...
    # ENV_NAME = 'taxi-random-0.3'
    # optimal_q_name = "OPTIMAL_Q_GAMMA_0.99_VALUE_0.15335.npy"
    ...
    resolutions = np.array([0.01, 0.05, 0.1, 0.5, 1.0, 3.0])
