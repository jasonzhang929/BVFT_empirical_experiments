...
def get_file_names(keywords, path=PATH):
...
def experiment1(model_keywords, data_keywords, num_models, data_sizes, resolutions):
    ...
    optimal_q = np.load(PATH + "OPTIMAL_Q_GAMMA_0.95_VALUE_0.086563.npy")
    q_functions[-1] = optimal_q
    values[-1] = 0.086563
    ...
    for j, data_size in enumerate(data_sizes):
        ...
        record = BvftRecord(data_size=data_size, gamma=0.95, data_explore_rate=data_explore_rate, model_count=num_models)
        record.model_values = values
        bvft = BVFT(q_functions, data, GAMMA, RMAX, RMIN, record, bins=bins, tabular=True)

        for i, res in enumerate(resolutions):
            ranks, loss_matrix, bin_histo, group_count = bvft.run(resolution=res)
            sorted_loss = np.sort(np.max(loss_matrix, axis=1))
            print(sorted_loss)
            ...
            plot_percent_bin_sizes(axs_bin[i, j], record, res, bins)
            plot_performance_and_q_vs_loss_scatter(axs_per[i, j], record, res, q_star_diff, left=left, right=right, bot=bot)
...
def run_experiment_2(num_runs):
    ...
    resolutions = {5000: [0.01, 0.1, 1.0, 2.0, 3.0],
                   500: [0.01, 0.1, 1.0, 2.0, 3.0],
                   50000: [0.01, 0.1, 1.0, 2.0, 3.0]}
...
def experiment3(model_count, folder="", auto_res=False):
    ...
    for data_explore_rate in data_explore_rates:
        ...
        fig, axs = get_subplots(len(data_sizes), 4 if auto_res else 2, F"{ENV_NAME}, {model_count} models, data exploration rate {data_explore_rate}, {text}")
        fig_res, axs_res = get_subplots(k, len(data_sizes), F"{ENV_NAME}, {model_count} models, data exploration rate {data_explore_rate}, {text}")
        for i, data_size in enumerate(data_sizes):
            record_matcher = BvftRecord(data_size=data_size, data_explore_rate=data_explore_rate, gamma=GAMMA, model_count=model_count)
            matched_records = []
            ...
            plot_top_k_metrics(axs[i][:2], matched_records, exclude_q_star=not include_q_star, plot_loc=plot_loc, auto_res=False)
            if auto_res:
...
def experiment4(num_model):
    ...
    fig_q_star_diff_bvft, axs_q_star_diff_bvft = get_subplots(res_size, len(data_sizes), title_prefix + "|Q - Q*| vs BVFT loss")
    fig_berror_vs_bvft, axs_berror_vs_bvft = get_subplots(res_size, len(data_sizes), title_prefix + "|Q - TQ| vs BVFT loss")
    fig_bin_percent, axs_bin_percent = get_subplots(res_size, len(data_sizes), title_prefix + "BVFT group size distributions")
    fig_bin_count, axs_bin_count = get_subplots(res_size, len(data_sizes), title_prefix + "BVFT group size distributions")
    ...
    for j, data_size in enumerate(data_sizes):
        data_start = np.random.randint(len(dataset) - data_size)
        data = dataset[data_start:data_start + data_size]
        record = BvftRecord(data_size=data_size, gamma=0.95, data_explore_rate=data_explore_rate, model_count=num_model)
        record.model_values = values
        record.q_star_diff = q_star_diff
...
def experiment5(num_model):
    ...
    for j, data_size in enumerate(data_sizes):
        data_start = np.random.randint(len(dataset) - data_size)
        data = dataset[data_start:data_start + data_size]
        record = BvftRecord(data_size=data_size, gamma=GAMMA, data_explore_rate=data_explore_rate, model_count=num_model)
        record.model_values = values
        record.q_star_diff = q_star_diff
        record.bellman_error = bellman_error
        bvft = BVFT(q_functions, data, GAMMA, RMAX, RMIN, record, bins=bins, tabular=True, verbose=True)
        ...
        plot_bvft_loss_vs_resolution_plot(axs_res[j], record, exclude_q_star=not include_q_star, model_count=mc, plot_loc=plot_loc)
...
if __name__ == '__main__':
    ...
    # model_names = get_file_names(model_keywords)
    # q_functions, values = get_models(model_names, n=10)
    # for i in range(10):
    #     print(i)
    #     print(values[i])
    #     print(q_functions[i][:10])
    #
