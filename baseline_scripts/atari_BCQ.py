...
def interact_with_environment(env, replay_buffer, is_atari, num_actions, state_dim, device, args, parameters, eval_episodes=50):
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if args.train_behavioral and (t + 1) % parameters["eval_freq"] == 0:
            ...
            print('Total time {:.5} mins, eval took {} secs'.format((time.time() - start_time)/60, time.time() - temp))
            # np.save(f"./results/behavioral_{setting}", evaluations)
            stats = F"{t+1}_{evaluations[-1]}"
            policy.save(f"./../models/DQN_{setting}_{stats}")

    # Save final policy
    if args.train_behavioral:
        policy.save(f"./../models/DQN_{setting}")
...
def eval_policy(policy, env_name, seed, eval_episodes=10):
    ...
    eval_env, _, _, _ = BCQ_utils.make_env(env_name, atari_preprocessing)
